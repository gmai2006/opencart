...
public class ModelToolUploadDAOImpl implements ModelToolUploadDAO {
  ...
  public Response getUpload() {
    ...
    try {
      String queryStr = BeanUtils.getQueryString("getUpload");

      Query query = em.createNativeQuery(queryStr, OcUpload.class);

      if (null == queryStr) {
        logger.error("query string NOT FOUND getUpload");
        return Response.serverError().entity("query string NOT FOUND getUpload").build();
      }
      ...
      if (null == list || list.isEmpty()) {
        logger.warn("not record returned for getUpload");
        return Response.status(Response.Status.NOT_FOUND).entity("no record returned for getUpload").build();
      }
      for (OcUpload o : list) {
        array.add(o.toJson());
      }
  ...
  public Response deleteUpload() {
    ...
    int result = -1;
    try {
      String queryStr = BeanUtils.getQueryString("deleteUpload");

      if (null == queryStr) {
        logger.error("query string NOT FOUND deleteUpload");
        return Response.serverError().entity("query string NOT FOUND deleteUpload").build();
      }
