...
*/
public class ModelExtensionOpenBayAmazonOrderServiceImpl implements ModelExtensionOpenBayAmazonOrderService {

  private final static Logger logger = Logger.getLogger(ModelExtensionOpenBayAmazonOrderServiceImpl.class.getName());
  private ModelExtensionOpenBayAmazonOrderDao dao;

  @Inject
  protected ModelExtensionOpenBayAmazonOrderServiceImpl(final ModelExtensionOpenBayAmazonOrderDao rhs) {
      requireNonNull(rhs);
      this.dao = dao;
  }

  /**
   source file name:ModelExtensionOpenBayAmazonOrder0Dto
   */
  @Override
  public List<ModelExtensionOpenBayAmazonOrder0Dto> getProductOptionsByVar(Integer product_option_value_id,String language_id) {
    ...
    final List<ModelExtensionOpenBayAmazonOrder0Dto> returnList = dao.getProductOptionsByVar(product_option_value_id,language_id);
  ...
  public List<Integer> addAmazonOrderProducts(String model,Integer order_id) {
    logger.info("addAmazonOrderProducts - entered");

    final List<Integer> returnList = dao.addAmazonOrderProducts(model,order_id);
