...
private final Logger logger = Logger.getLogger(this.getClass().getName());
  ...
  @Inject
  protected OcLanguageServiceImpl(final OcLanguageDao dao) {
      requireNonNull(dao);
      this.dao = dao;
  }
  ...
  public List<OcLanguage> selectAll() {
      final List<OcLanguage> result = dao.selectAll();
      logger.info("selectAll(OcLanguage) - exited - return value={} result ");
      return result;
  }
  ...
  public void delete(OcLanguage bean) {
    logger.info("delete(OcLanguage={}) - entered bean");

    dao.delete(bean);

    logger.info("delete(OcLanguage) - exited - return value={} result ");
  }
}