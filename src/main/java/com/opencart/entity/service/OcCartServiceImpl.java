...
*/
public class OcCartServiceImpl implements OcCartService {
private final Logger logger = Logger.getLogger(this.getClass().getName());

  private final OcCartDao dao;

  @Inject
  protected OcCartServiceImpl(final OcCartDao dao) {
      requireNonNull(dao);
      this.dao = dao;
  }
  ...
  public List<OcCart> selectAll() {
      final List<OcCart> result = dao.selectAll();
      logger.info("selectAll(OcCart) - exited - return value={} result ");
      return result;
  }
  ...
  @Override
  @Transactional
  public void delete(OcCart bean) {
    logger.info("delete(OcCart={}) - entered bean");

    dao.delete(bean);

    logger.info("delete(OcCart) - exited - return value={} result ");
  }
}