getProductStatus5=SELECT * FROM  oc_amazon_product  WHERE  product_id  = ?1 AND version = 3
#END
uninstall1=DROP TABLE IF EXISTS  oc_amazon_order_product
#END
...
getUnlinkedItemsFromReport=
				...
				WHERE apl.product_id IS NULL AND alr.marketplace = ?2
				ORDER BY alr.sku
				LIMIT ?3,?4
#END
getProductErrors=SELECT  messages  FROM  oc_amazon_product  WHERE  product_id  = ?1 AND version = 3
#END
getProductsWithErrors=
			SELECT  product_id ,  sku  FROM  oc_amazon_product
			WHERE status = 'error' AND version = 2
#END
updateAmazonSkusQuantities1=SELECT apl.amazon_sku, p.quantity FROM oc_amazon_product_link apl JOIN oc_product p ON apl.product_id = p.product_id WHERE apl.amazon_sku IN (?1)
#END
uninstall8=DROP TABLE IF EXISTS  oc_amazon_product_unshipped
#END
uninstall9=DROP TABLE IF EXISTS  oc_amazon_product_search
#END
uninstall6=DROP TABLE IF EXISTS  oc_amazon_product_error
#END
uninstall7=DROP TABLE IF EXISTS  oc_amazon_process
#END
uninstall4=DROP TABLE IF EXISTS  oc_amazon_product_link
#END
uninstall5=DROP TABLE IF EXISTS  oc_amazon_product_unshipped
#END
setProductUploaded=UPDATE  oc_amazon_product 
			SET  status  = 'uploaded',  insertion_id  = ?1
			WHERE  product_id  = ?2 AND  var  = ?3 AND version = 2
#END
deleteProduct=DELETE FROM  oc_amazon_product 
			WHERE  product_id  = ?1
#END
uninstall2=DROP TABLE IF EXISTS  oc_amazon_product2
#END
uninstall3=DROP TABLE IF EXISTS  oc_amazon_product
#END
getOrderStatusString=
			...
			LIMIT 1
#END
removeProductLink=DELETE FROM  oc_amazon_product_link 
			WHERE  amazon_sku  = ?1
#END
...
#END
uninstall11=DELETE FROM  oc_setting WHERE code = 'openbay_amazon'
#END
uninstall10=DROP TABLE IF EXISTS  oc_amazon_listing_report
#END
linkProduct1=INSERT INTO  oc_amazon_product_link  SET  product_id  = ?1,  amazon_sku  = ?2,  var  = ?3
#END
uninstall=DROP TABLE IF EXISTS  oc_amazon_order
#END
getAmazonOrderedProducts=
			...
			JOIN  oc_order_product   op  ON  op . order_product_id  =  aop . order_product_id 
				AND  op . order_id  = ?1
#END
getProductErrors2=SELECT * FROM  oc_amazon_product_error  WHERE  sku  = ?1 AND  insertion_id  = ?2
#END
removeAdvancedErrors2=UPDATE  oc_amazon_product 
			SET  status  = 'saved',  insertion_id  = ''
			WHERE  product_id  = ?1 AND status = 'error' AND version = 2
#END
getProductLinks=
#END
getProductErrors1=SELECT  sku ,  insertion_id  FROM  oc_amazon_product  WHERE  product_id  = ?1 AND version = 2
#END
removeAdvancedErrors1=DELETE FROM  oc_amazon_product_error 
...
#END
productLinkExists=SELECT * FROM  oc_amazon_product_link  WHERE  product_id  = ?1 AND var = ?2
#END
deleteListingReports=DELETE FROM oc_amazon_listing_report WHERE marketplace = ?1
#END
getSavedProducts=
...
#END
deleteSaved=DELETE FROM  oc_amazon_product 
...
#END
updateAmazonSkusQuantities=SELECT apl.amazon_sku, if (pov.product_id IS NULL, p.quantity, pov.stock) AS 'quantity' FROM oc_amazon_product_link apl JOIN oc_product p ON apl.product_id = p.product_id LEFT JOIN oc_product_option_variant pov ON apl.product_id = pov.product_id AND apl.var = pov.sku WHERE apl.amazon_sku IN (?1)
#END
getProductCategory=
...
#END
resetUploaded=UPDATE  oc_amazon_product 
			SET  status  = 'saved',  insertion_id  = ''
			WHERE  insertion_id  = ?1 AND version = 2
#END
updateAmazonOrderTracking=UPDATE  oc_amazon_order  SET  courier_id  = ?1,  courier_other  = ?2,  tracking_no  = ?3 WHERE  order_id  = ?4
#END
