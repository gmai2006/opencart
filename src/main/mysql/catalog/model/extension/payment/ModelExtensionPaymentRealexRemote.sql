addOrder=INSERT INTO  oc_realex_remote_order  SET  order_id  = ?1,  settle_type  = ?2,  order_ref  = ?3,  order_ref_previous  = ?4,  date_added  = now(),  date_modified  = now(),  capture_status  = ?5,  currency_code  = ?6,  pasref  = ?7,  pasref_previous  = ?8,  authcode  = ?9,  account  = ?10,  total  = ?11
#END
