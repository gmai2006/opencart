getOrderId=SELECT  order_id  FROM  oc_amazonus_order  WHERE  amazonus_order_id  = ?1 LIMIT 1
#END
...
#END
getProductId=SELECT  product_id  FROM  oc_amazonus_product_link  WHERE  amazonus_sku  = ?1
#END
...
#END
addAmazonusOrderProducts=SELECT  order_product_id  FROM  oc_order_product  WHERE  model  = ?1 AND  order_id  = ?2LIMIT 1
#END
getProductVar=SELECT  var  FROM  oc_amazonus_product_link  WHERE  amazonus_sku  = ?1
#END
addAmazonusOrder=INSERT INTO  oc_amazonus_order  ( order_id ,  amazonus_order_id ) VALUES (?1, ?2)
#END
...
#END
addAmazonusOrderProducts1=INSERT INTO  oc_amazonus_order_product  ( order_product_id ,  amazonus_order_item_id ) VALUES (?1, ?2)
#END
getCountryName=SELECT  name  FROM  oc_country  WHERE LOWER( iso_code_2 ) = ?1
#END
getAmazonusOrderId=SELECT  amazonus_order_id  FROM  oc_amazonus_order  WHERE  order_id  = ?1 LIMIT 1
#END
