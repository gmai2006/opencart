getProductStatus5=$this->db->query("SELECT * FROM `".DB_PREFIX."amazon_product` WHERE `product_id` = ".(int)$product_id." AND `version` = 3")->row
#END
uninstall1=$this->db->query("DROP TABLE IF EXISTS `".DB_PREFIX."amazon_order_product`")
#END
...
#END
getProductErrors=$this->db->query("SELECT `messages` FROM `".DB_PREFIX."amazon_product` WHERE `product_id` = '".(int)$product_id."' AND `version` = 3")->row
#END
...
#END
updateAmazonSkusQuantities1=$this->db->query("SELECT apl.amazon_sku, p.quantity FROM ".DB_PREFIX."amazon_product_link apl JOIN ".DB_PREFIX."product p ON apl.product_id = p.product_id WHERE apl.amazon_sku IN (".implode(',',$sku_array).")")->rows
#END
uninstall8=$this->db->query("DROP TABLE IF EXISTS `".DB_PREFIX."amazon_product_unshipped`")
#END
uninstall9=$this->db->query("DROP TABLE IF EXISTS `".DB_PREFIX."amazon_product_search`")
#END
uninstall6=$this->db->query("DROP TABLE IF EXISTS `".DB_PREFIX."amazon_product_error`")
#END
uninstall7=$this->db->query("DROP TABLE IF EXISTS `".DB_PREFIX."amazon_process`")
#END
uninstall4=$this->db->query("DROP TABLE IF EXISTS `".DB_PREFIX."amazon_product_link`")
#END
uninstall5=$this->db->query("DROP TABLE IF EXISTS `".DB_PREFIX."amazon_product_unshipped`")
#END
setProductUploaded=$this->db->query("UPDATE `".DB_PREFIX."amazon_product`
...
#END
deleteProduct=$this->db->query("DELETE FROM `".DB_PREFIX."amazon_product`
			WHERE `product_id` = '".(int)$product_id."'")
#END
uninstall2=$this->db->query("DROP TABLE IF EXISTS `".DB_PREFIX."amazon_product2`")
#END
uninstall3=$this->db->query("DROP TABLE IF EXISTS `".DB_PREFIX."amazon_product`")
#END
getOrderStatusString=$this->db->query("
			...
			LIMIT 1")->row
#END
removeProductLink=$this->db->query("DELETE FROM `".DB_PREFIX."amazon_product_link`
...
getUnlinkedProducts=$this->db->query("
				...
				AND `pd`.`language_id` = '".(int)$this->config->get('config_language_id')."'")->rows
#END
getAmazonOrderId=$this->db->query("
...
#END
uninstall11=$this->db->query("DELETE FROM `".DB_PREFIX."setting` WHERE `code` = 'openbay_amazon'")
#END
uninstall10=$this->db->query("DROP TABLE IF EXISTS `".DB_PREFIX."amazon_listing_report`")
#END
...
#END
uninstall=$this->db->query("DROP TABLE IF EXISTS `".DB_PREFIX."amazon_order`")
#END
getAmazonOrderedProducts=$this->db->query("
...
#END
getProductErrors2=$this->db->query("SELECT * FROM `".DB_PREFIX."amazon_product_error` WHERE `sku` = '".$this->db->escape($insertion_row['sku'])."' AND `insertion_id` = '".$this->db->escape($insertion_row['insertion_id'])."'")->rows
#END
removeAdvancedErrors2=$this->db->query("UPDATE `".DB_PREFIX."amazon_product`
...
#END
getProductErrors1=$this->db->query("SELECT `sku`, `insertion_id` FROM `".DB_PREFIX."amazon_product` WHERE `product_id` = '".(int)$product_id."' AND `version` = 2")->rows
#END
removeAdvancedErrors1=$this->db->query("DELETE FROM `".DB_PREFIX."amazon_product_error`
...
#END
productLinkExists=$this->db->query("SELECT * FROM `".DB_PREFIX."amazon_product_link` WHERE `product_id` = ".(int)$product_id." AND var = '".$this->db->escape($var)."'")->row
#END
deleteListingReports=$this->db->query("DELETE FROM ".DB_PREFIX."amazon_listing_report WHERE marketplace = '".$this->db->escape($marketplace)."'")
#END
...
#END
deleteSaved=$this->db->query("DELETE FROM `".DB_PREFIX."amazon_product`
...
#END
resetUploaded=$this->db->query("UPDATE `".DB_PREFIX."amazon_product`
			SET `status` = 'saved', `insertion_id` = ''
			WHERE `insertion_id` = '".$this->db->escape($insertion_id)."' AND `version` = 2")
#END
updateAmazonOrderTracking=$this->db->query("UPDATE `".DB_PREFIX."amazon_order` SET `courier_id` = '".$this->db->escape($courier_id)."', `courier_other` = ".(int)!$courier_from_list.", `tracking_no` = '".$this->db->escape($tracking_no)."' WHERE `order_id` = ".(int)$order_id."")
#END
