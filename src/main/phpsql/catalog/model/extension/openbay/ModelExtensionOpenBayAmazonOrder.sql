addAmazonOrderProducts=$this->db->query("SELECT `order_product_id` FROM `".DB_PREFIX."order_product` WHERE `model` = '".$this->db->escape($sku)."' AND `order_id` = ".(int)$order_id." LIMIT 1")->row
#END
getOrderId=$this->db->query("SELECT `order_id` FROM `".DB_PREFIX."amazon_order` WHERE `amazon_order_id` = '".$this->db->escape($amazon_order_id)."' LIMIT 1")->row
#END
...
#END
getAmazonOrderId=$this->db->query("SELECT `amazon_order_id` FROM `".DB_PREFIX."amazon_order` WHERE `order_id` = ".(int)$order_id." LIMIT 1")->row
#END
...
#END
addAmazonOrder=$this->db->query("INSERT INTO `".DB_PREFIX."amazon_order` (`order_id`, `amazon_order_id`) VALUES (".(int)$order_id.", '".$this->db->escape($amazon_order_id)."')")
#END
addAmazonOrderProducts1=$this->db->query("INSERT INTO `".DB_PREFIX."amazon_order_product` (`order_product_id`, `amazon_order_item_id`) VALUES (".(int)$order_product_id.", '".$this->db->escape($order_item_id)."')")
#END
getProductId=$this->db->query("SELECT `product_id` FROM `".DB_PREFIX."amazon_product_link` WHERE `amazon_sku` = '".$this->db->escape($sku)."'")->row
#END
...
#END
getProductVar=$this->db->query("SELECT `var` FROM `".DB_PREFIX."amazon_product_link` WHERE `amazon_sku` = '".$this->db->escape($sku)."'")->row
#END
